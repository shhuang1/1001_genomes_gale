...
this_analysis_path = file.path(PROJ_RESULTS_PATH,"acc_variable_genes_2016-02-05")
prefix.string = file.path(this_analysis_path,"graphics","acc_variable_genes_2016-02-05-")
rdata_prefix = file.path(this_analysis_path,'acc_variable_genes_2016-02-05-')
step_list = ""
@
...
opts_chunk$set(fig.path=prefix.string, fig.align='center', fig.show='hold',
               ...
               cache.path = 'cache/acc_variable_genes_2016-02-05-')
...
if ("save_deseq_results" %in% step_list) {
   save(ddsMat,res,file=paste0(rdata_prefix,'deseq_res.RData'))
}
@ 
...
percentVar <- round(100 * attr(pca_dat, "percentVar"))
p4a = ggplot(data=pca_dat,aes(PC1, PC2, color=(group=='relic'))) 
p4b = p4a + geom_point(size=3)
p4c = p4b + 
  scale_color_brewer(palette='Set1') +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
