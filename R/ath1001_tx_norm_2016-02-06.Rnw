...
this_analysis_path = file.path(PROJ_RESULTS_PATH,"ath1001_tx_norm_2016-02-06")
prefix.string = file.path(this_analysis_path,"graphics","ath1001_tx_norm_2016-02-06-")
rdata_prefix = file.path(this_analysis_path,"ath1001_tx_norm_2016-02-06_2-")
...
opts_chunk$set(fig.path=prefix.string, fig.align='center', fig.show='hold',
               ...
               cache.path="cache/ath1001_tx_norm_2016-02-06_2-")
...
\item Run RUVg on DESeq normalized counts (http://master.bioconductor.org/help/workflows/rnaseqGene/\#batch)
\end{itemize}
...
if ("run_ruvg_21ref" %in% step_list) {
    ...
    RUVg_21ref_uq = alply(try_k,1,function(k) {
        ...
        normCounts = cbind(gene_id=rownames(normCounts),normCounts)
        if ("save_results" %in% step_list) {
            write.table(W,paste0(rdata_prefix,'UQ_gNorm_W_k',k,'.txt'),sep='\t',
                        row.names=FALSE,col.names=TRUE,quote=FALSE)    
            write.table(normCounts,paste0(rdata_prefix,'UQ_gNorm_normCounts_k',k,'.txt'),sep='\t',
                        row.names=FALSE,col.names=TRUE,quote=FALSE)
        }
        return(gNorm)
    },.parallel=TRUE)
...
if ("run_ruvg_21ref" %in% step_list) {
    ...
    RUVg_21ref_uqRIN = alply(try_k,1,function(k) {
        ...
        normCounts = cbind(gene_id=rownames(normCounts),normCounts)
        if ("save_results" %in% step_list) {
            write.table(W,paste0(rdata_prefix,'UQRIN_gNorm_W_k',k,'.txt'),sep='\t',
                        row.names=FALSE,col.names=TRUE,quote=FALSE)    
            write.table(normCounts,paste0(rdata_prefix,'UQRIN_gNorm_normCounts_k',k,'.txt'),sep='\t',
                        row.names=FALSE,col.names=TRUE,quote=FALSE)
        }
        return(gNorm)
    },.parallel=TRUE)
...
for (k in try_kL) {   
    ...
    write.table(vsd_matL[[k]],
               paste0(rdata_prefix,sprintf('UQ_gNorm_normCounts_k%s_vst2.txt',k)),
...
for (k in try_kL) {
    write.table(vsd_mat_cv1L[[k]],
                paste0(rdata_prefix,sprintf('UQ_gNorm_normCounts_k%s_vst2_cv0p05.txt',k)),
                quote=F,sep='\t')   
    ...
    write.table(vsd_mat_cv1L[[k]][,accx2_1001g],
                paste0(rdata_prefix,sprintf('UQ_gNorm_normCounts_k%s_vst2_cv0p05_1001g.txt',k)),
                quote=F,sep='\t')   
    ...
    write.table(uq_mat_cv1L[[k]],
                paste0(rdata_prefix,sprintf('UQ_gNorm_k%s_vst2_cv0p05_UQCounts.txt',k)),
                quote=F,sep='\t')   
    ...
    write.table(uq_mat_cv1L[[k]][,accx2_1001g],
                paste0(rdata_prefix,sprintf('UQ_gNorm_k%s_vst2_cv0p05_UQCounts_1001g.txt',k)),
                quote=F,sep='\t')   
    ...
    write.table(uqRIN_mat_cv1L[[k]],
                paste0(rdata_prefix,sprintf('UQRIN_gNorm_k%s_vst2_cv0p05_UQRIN.txt',k)),
                quote=F,sep='\t')   
    ...
    write.table(uqRIN_mat_cv1L[[k]][,accx2_1001g],
                paste0(rdata_prefix,sprintf('UQRIN_gNorm_k%s_vst2_cv0p05_UQRIN_1001g.txt',k)),
                quote=F,sep='\t')   
    write.table(t(uqRIN_mat_cv1L[[k]][,accx2_1001g]),
     ...
     write.table(count_mat_cv1L[[k]],
                paste0(rdata_prefix,sprintf('UQ_gNorm_k%s_vst2_cv0p05_RawCounts_1001g.txt',k)),
                quote=FALSE,sep='\t')
...
for (k in try_kL) {
    ...
    write.table(vsd_mat_rin1L[[k]],
            paste0(rdata_prefix,sprintf('UQ_gNorm_normCounts_k%s_vst2_rin.txt',k)),
            quote=F,sep='\t')
}
...
for (k in try_kL) {
    ...
    write.table(vsd_mat_cv1_rin1L[[k]],
                paste0(rdata_prefix,sprintf('UQ_gNorm_normCounts_k%s_vst2_cv0p05_rin.txt',k)),
                quote=F,sep='\t')
