...
this_analysis_path = file.path(PROJ_RESULTS_PATH,"eqtl_general_stats_2016-05-19")
prefix.string = file.path(this_analysis_path,"graphics","eqtl_general_stats_2016-05-19-")
rdata_prefix = file.path(this_analysis_path,"eqtl_general_stats_2016-05-19-")
step_list = ""
@
...
opts_chunk$set(fig.path=prefix.string, fig.align='center', fig.show='hold',
               ...
               cache.path="cache/eqtl_general_stats_2016-05-19-")
...
\item Using eQTL test results in \verb|\Sexpr{eqtl_19_path}|.
\end{itemize}
...
eqtl_k = expand.grid(eqtln=c(16,19))

k_master = rbindlist(alply(eqtl_k,1,function(ei) {
  dt = fread(file.path(PROJ_RESULTS_PATH,sprintf('marginal_test_cov_%s/results/lmm_master/chr1-5genes_1e-5.bed.sorted',ei[1,"eqtln"])))
  cbind(data.table(ei),dt)
}))

k_thr_numgenes = adply(thr_info,1,function(thr) {
  unique(k_master[V5<=thr[1,'thr_val'],],by=c("eqtln","V4"))[,list(num_genes=.N),by=c("eqtln")]
})

k_thr_uniqpos = adply(thr_info,1,function(thr) {
  unique(k_master[V5<=thr[1,'thr_val'],],by=c("V1","V2","eqtln"))[,list(num_pos=.N),by=c("eqtln")]
})
@
...
p1a = ggplot(data=k_thr_numgenes,aes(x=thr_val,y=num_genes,group=factor(eqtln),color=factor(eqtln)))
p1b = p1a + geom_line() + geom_point(size=0.5)
p1c = p1b + scale_x_log10("P-value threshold") +
...
p2a = ggplot(data=k_thr_uniqpos,aes(x=thr_val,y=num_pos,group=factor(eqtln),color=factor(eqtln)))
p2b = p2a + geom_line() + geom_point(size=0.5) 
p2c = p2b + scale_x_log10("P-value threshold") +
...
num_genes_pos = merge(k_thr_numgenes,k_thr_uniqpos,by=c("thr_name","thr_val","eqtln"))
...
p3a = ggplot(data=num_genes_pos,aes(x=num_pos,y=num_genes))
p3b = p3a + geom_point(size=0.5) + geom_line(size=0.4) +
  facet_wrap(~eqtln,ncol=4)
p3c = p3b + scale_x_continuous("Number of positions",trans = log_trans(), breaks = base_breaks()) +
  scale_y_continuous("Number of genes") +
  theme_bw() +
...
p4c = p4b + scale_color_brewer("",palette="Set2",
    ...
    scale_x_continuous("Distance from local eQTL to gene (kb)") +
    scale_y_continuous("Count",breaks=seq(0,800,by=200),limits=c(0,800)) +
    theme_bw() +
    theme(axis.title.x=element_text(size=6),
          axis.text.x=element_text(size=5),
          axis.title.y=element_text(size=6),
...
<<tss_grp1_bar,fig.width=2.2,fig.height=1.8>>=
tss_dt3 = tss_dt[,dist_tss_grp1:=ifelse(is.na(dist_tss),rep('diff_chr',.N),
                                 ifelse(abs(dist_tss)<10000,rep('prox',.N),rep('dist',.N)))]
tss_dt3[,dist_tss_grp1:=factor(dist_tss_grp1,levels=c('prox','dist','diff_chr'))]
p5a = ggplot(data=tss_dt3,aes(x=allele,fill=dist_tss_grp1))
p5b = p5a + geom_bar(position="dodge")
p5c = p5b + scale_x_discrete("",limits=c('eQTL','eQTL_mCG','eQTL_mCH','eQTL_mC'),
                             breaks=c('eQTL','eQTL_mCG','eQTL_mCH','eQTL_mC'),
                             labels=c("eQTL",expression(paste("eQTL"^"CG-DMB")),
                                      expression(paste("eQTL"^"CH-DMB")),
                                      expression(paste("eQTL"^"C-DMB")))) +
    scale_fill_brewer("",palette="Set2",
                      breaks=c('prox','dist','diff_chr'),
                      labels=c('Local (< 10 kb)','Distant (>= 10kb)','Different chromosome')) +
...
#pdf(paste0(prefix.string,'aaa.pdf'),width=2.2,height=1.8)
print(p5c)
#dev.off()
@ 
