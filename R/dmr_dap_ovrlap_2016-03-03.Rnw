...
this_analysis_path = file.path(PROJ_RESULTS_PATH,"dmr_dap_orlap_2016-03-03")
prefix.string = file.path(this_analysis_path,"graphics","dmr_dap_orlap_2016-03-03-")
step_list = ""
@
...
opts_chunk$set(fig.path=prefix.string, fig.align='center', fig.show='hold',
               ...
               cache.path="cache/dmr_dap_orlap_2016-03-03-")
...
\item DMR overlap with DAP sites association testing in \verb|\Sexpr{dmc_bins_gat01_path}|.
\end{itemize}
...
gat_list = alply(feat_names,1,function(feat) {
  feat_list = rbindlist(alply(dmc_bins_filtered,1,function(b) {
    gat_file = file.path(dmc_bins_gat01_path,b,paste0(b,'.gat_',feat,'.out'))
    dt = fread(gat_file)
...
dmc_feat_names = c('dapfimo2','dapfimo3')
...
dms_feat_names = c('dapfimo2','dapfimo3')
...
snp_feat_names = c('dapfimo2','dapfimo3')
...
<<gat_dap1,fig.cap="Log2 fold enrichment of dmC bins with family DAP peaks or motifs in peaks.  Pattern is consistent between peaks and motifs in peaks, but the effect is stronger in motifs.  Note relatively little enrichment when all binding sites are pooled together (\"master\").">>=
...
p1c = p1b + scale_fill_gradientn("log2\nfold",
    colours=tim.colors(50),limits=c(-max_l2f_dap,max_l2f_dap),
    na.value="grey75") + 
    ...
    theme(legend.position="bottom",
          ...
          axis.gtitle.x=element_text(size=6),
...
<<gat_dap_hm1,fig.width=1.8,fig.height=3.6,fig.caption="Heatmap showing only log2 enrichment with motif matches in peaks">>=
dap_l2f_wide0 = reshape(dap_l2f_plot[feat=='dapfimo1' & annotation!='master' & annotation!='Homeobox_ecoli',
...
#pdf(paste0(prefix.string,'gat_dap_hm1.pdf'),width=1.8,height=3.6)
aheatmap(dap_l2f_wide1,breaks=0,col=tim.colors(50),
         treeheight=15,
         Colv=NA)
#dev.off()
@

<<gat_dap2_hm,fig.width=6,fig.height=6>>=
exp_gat_dt0 = rbindlist(c(dmc_gat_list,dms_gat_list,snp_gat_list))
...
#pdf(paste0(prefix.string,'aa1.pdf'),width=8,height=8)
aheatmap(exp_gat_wide[,-c("annotation"),with=FALSE],breaks=0)
#dev.off()
@ 

<<mc_cor_hm,fig.width=5,fig.height=5>>=
mc_cor = cor(exp_gat_wide[,-"annotation",with=FALSE],use="pairwise.complete.obs")

#pdf(paste0(prefix.string,'aa2.pdf'),width=5,height=5)
aheatmap(mc_cor,col="Reds")
#dev.off()
@ 

<<gat_dapfc_scatter,fig.width=1.4,fig.height=1.4>>=
p2a = ggplot(data=exp_gat_wide0)
p2b = p2a + geom_point(aes(x=l2fold.dmCH_filtered.dapfimo3,y=log2fc.dap.MR_mC),alpha=0.5,size=0.2)
p2c = p2b + scale_x_continuous(expression(paste("log2 TFBS FE in mCH eQTL" ^"epi"))) +
...
#pdf(paste0(prefix.string,'aa3.pdf'),width=1.4,height=1.4)
print(p2c)
#dev.off()

#pdf(paste0(prefix.string,'aa3.pdf'),width=1.4,height=1.4)
p2c + geom_point(aes(x=l2fold.snp.dapfimo3,y=log2fc.dap.MR_mC),alpha=0.5,size=0.5) +
    scale_x_continuous("log2 TFBS FE in eQTL")
#dev.off()
@ 

<<gat_dapfc_scatter2,fig.width=2.4,fig.height=1.4>>=
scatter2_plot = rbindlist(list(exp_gat_wide0[l2fold.dmCH_filtered.dapfimo3>-6,list(x=log2fc.dap.MR_mC,y=l2fold.dmCH_filtered.dapfimo3,allele='eQTL epi')],
                               exp_gat_wide0[,list(x=log2fc.dap.MR_mC,y=l2fold.snp.dapfimo3,allele='eQTL')]))
scatter2_plot[,allele:=factor(allele,levels=c("eQTL epi","eQTL"))]
mc_cor_plot = data.table('allele'=c("eQTL epi","eQTL"),
                         x=c(3,3),y=c(-4,-3),
                         pcor=mc_cor['log2fc.dap.MR_mC',c('l2fold.dmCH_filtered.dapfimo3','l2fold.snp.dapfimo3')])
mc_cor_plot[,allele:=factor(allele,levels=c("eQTL epi","eQTL"))]
...
p3c = p3b + scale_x_continuous(expression(paste("log2 fold binding inhibition by mCH"))) +
...
#pdf(paste0(prefix.string,'aa4.pdf'),width=2.4,height=1.4)
print(p3c)
#dev.off()
@ 


<<exp_gat_hm2,fig.width=1.8,fig.height=2.4>>=
exp_gat_hm2_plot = exp_gat_wide[!is.na(log2fc.dap.MR_mC) & log2fc.dap.MR_mC <=-1 & l2fold.dmCH_filtered.dapfimo3 >= log2(1.5),]
exp_gat_hm2_mat = as.matrix(exp_gat_hm2_plot[,list("Binding inhibition"=log2fc.dap.MR_mC,
                                                   "eQTLepi enrichment"=l2fold.dmCH_filtered.dapfimo3,
                                                   "eQTL enrichment"=l2fold.snp.dapfimo3)])
...
aheatmap(exp_gat_hm2_mat,Colv=NA,col=tim.colors(50),breaks=0,cexCol=0.6,treeheight=15)
#dev.off()
@ 

<<exp_gat_hm3,fig.width=1.8,fig.height=2.4>>=
exp_gat_hm3_rank = exp_gat_wide[,list(dmCH_filtered.rank=rank(-l2fold.dmCH_filtered.dapfimo3),snp.rank=rank(-l2fold.snp.dapfimo3))]
exp_gat_hm3_plot = exp_gat_wide[exp_gat_hm3_rank[,(dmCH_filtered.rank<=10 | snp.rank<=10) & !(is.na(log2fc.dap.MR_mC))],]
                                
exp_gat_hm3_mat = as.matrix(exp_gat_hm3_plot[,list("eQTL enrichment"=l2fold.snp.dapfimo3,
                                                   "eQTLepi enrichment"=l2fold.dmCH_filtered.dapfimo3,
...
#pdf(paste0(prefix.string,'aa6.pdf'),width=1.8,height=2.4)                   
aheatmap(exp_gat_hm3_mat,Colv=NA,col=tim.colors(50),breaks=0,cexCol=0.6,treeheight=15,na.color="grey50")
#dev.off()
@ 

<<exp_gat_gVsEpi,fig.width=2.2,fig.height=1.2>>=
exp_gat_gVsEpi = exp_gat_wide[!is.na(log2fc.dap.MR_mC),]
max_l2f_dap = max(abs(exp_gat_gVsEpi[,log2fc.dap.MR_mC])+0.01)

dapfimo_quantiles = rbindlist(alply(c("l2fold.snp.dapfimo3","l2fold.dmCH_filtered.dapfimo3"),
                          1,function(i) {
                              data.table(l2fold=i,
...
p4a = ggplot(data=exp_gat_gVsEpi,aes(x=l2fold.snp.dapfimo3,
                                     y=l2fold.dmCH_filtered.dapfimo3,
                                     color=log2fc.dap.MR_mC))
p4b = p4a + geom_point(size=0.1,alpha=0.7) +
    geom_hline(yintercept=dapfimo_quantiles[l2fold=='l2fold.dmCH_filtered.dapfimo3',V2],
               linetype='solid',size=0.25) +
    geom_vline(xintercept=dapfimo_quantiles[l2fold=='l2fold.snp.dapfimo3',V2],
               linetype='longdash',size=0.25) 
p4c = p4b + scale_x_continuous("log2 FE in eQTL") +
    scale_y_continuous(expression(paste("log2 FE in eQTL"^"CH-DMR"))) +
...
<<exp_gat_epiVsCIC,fig.width=1.3,fig.height=1.2>>=
exp_gat_gVsEpi = exp_gat_wide[!is.na(log2fc.dap.MR_mC),]
...
p5b = p5a + geom_point(aes(x=icsum.cg,y=l2fold.dmCH_filtered.dapfimo3),
                       size=0.01,alpha=0.5,shape=1) 
p5c = p5b + scale_x_continuous("Motif CG content") +
    scale_y_continuous(expression(paste("log2 FE in eQTL"^"CH-DMR"))) +
    theme_bw() + 
    theme(axis.text.x=element_text(size=5),
          axis.text.y=element_text(size=5),
...
for (mc_prefix in dmc_bins_filtered) {
    ...
    cistrome_table = fread(file.path(cistrome_olap01_path,mc_prefix,paste0(mc_prefix,'_cistrome_table.txt')),header=FALSE)
...
dmC_all_table = fread(file.path(cistrome_olap01_path,'dmC_all/dmC_all_cistrome_table.txt'),header=FALSE)
