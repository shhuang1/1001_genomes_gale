...
this_analysis_path = file.path(PROJ_RESULTS_PATH,"ath1001_tx_norm_2016-01-03_2")
prefix.string = file.path(this_analysis_path,"graphics","ath1001_tx_norm_2016-01-03_2-")
rdata_prefix = file.path(this_analysis_path,"ath1001_tx_norm_2016-01-03_2-")
step_list = ""
@
...
opts_chunk$set(fig.path=prefix.string, fig.align='center', fig.show='hold',
               ...
               cache.path="cache/ath1001_tx_norm_2016-01-03_2-")
...
\section{Load raw counts and non-specific filtering}
...
\section{Default DESeq2 normalization}

\subsection{Genes passed non-specific filter,and accesions overlapping with 1001g}
...
if ("load_vst" %in% step_list) {
  load(paste0(rdata_prefix,'gNormCounts_vsd_blind_list.RData'))
} else {
  ...
  save(vsdL,ddsMatL,deseq_ncountsL,file=paste0(rdata_prefix,'gNormCounts_vsd_blind_list.RData'))
}
...
hts_count2_cv1L = llply(try_kL,function(k) hts_count2[rownames(vsd_mat_cv1L[[k]]),])

for (k in try_k) {    
    write.table(vsd_mat_cv1L[[k]],
                paste0(rdata_prefix,sprintf('gNorm_normCounts_k%s_1001g_vst2_cv0p05.txt',k)),
                quote=F,sep='\t')   
    write.table(hts_count2_cv1L[[k]],
                paste0(rdata_prefix,sprintf('gNorm_k%s_vst2_cv0p05_RawCounts.txt',k)),
                quote=F,sep='\t')   
    write.table(t(hts_count2_cv1L[[k]]),
                paste0(rdata_prefix,sprintf('gNorm_k%s_vst2_cv0p05_RawCountsT.csv',k)),
                quote=FALSE,sep=',',col.names=NA,row.names=TRUE)
    write.table(hts_count2_cv1L[[k]][,accx2_1001g],
                paste0(rdata_prefix,sprintf('gNorm_k%s_vst2_cv0p05_RawCounts_1001g.txt',k)),
                quote=F,sep='\t')   
    write.table(t(hts_count2_cv1L[[k]][,accx2_1001g]),
                paste0(rdata_prefix,sprintf('gNorm_k%s_vst2_cv0p05_RawCounts_1001gT.csv',k)),
                quote=FALSE,sep=',',col.names=NA,row.names=TRUE)

}
@ 

<<rankit_try_k,eval=FALSE>>=
