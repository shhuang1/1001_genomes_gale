...
counts_mat_file = file.path(PROJ_RESULTS_PATH,"ath1001_tx_norm_2016-02-06",
                           "ath1001_tx_norm_2016-02-06-filtered01_1001g.csv")
...
this_analysis_path = file.path(PROJ_RESULTS_PATH,"acc_variable_genes_2016-02-29")
prefix.string = file.path(this_analysis_path,"graphics","acc_variable_genes_2016-02-29-")
rdata_prefix = file.path(this_analysis_path,'acc_variable_genes_2016-02-29-')
step_list = "load_lrt"
@
...
<<topDEGenes_list>>=
topDEGenes_list = lapply(topTable_list,function(tt) {
    mapply(function(tt_k,tg_k) {
      list(top1=rownames(tt_k[1:length(tg_k),]),top2=rownames(subset(tt_k,PValue<=0.05)))
    },tt,topVarGenes,SIMPLIFY=FALSE)
})

topDEGenes_list = unlist(unlist(topDEGenes_list,recursive=FALSE),recursive=FALSE)
topDEGenes_list = topDEGenes_list[lapply(topDEGenes_list,length)<=5000]
@
...
p8a = ggplot(subset(deg_count,name.exp!='allgroups'),
...
p8a = ggplot(subset(deg_count,name.exp!='allgroups' & k==4),aes(x=name.exp,y=V1)) 
...
p8c = p8b + scale_x_discrete("",limits=c('rvnr','avr','avnr'),
                             labels=c("Relict vs\nNon-relict",
                                      "Admixed vs\nRelict",
                                      "Admixed vs\nNon-relict")) +
    scale_y_continuous("Number of diff. exp. genes",limits=c(0,10750),breaks=seq(0,9000,by=3000)) +
    theme_bw() +
    theme(axis.title.x=element_text(size=6),
...
deg_k4_list = llply(topTable_list,function(tt) {
    tt_k = tt[['4']]
    rownames(subset(tt_k,PValue<=0.05))
})

#pdf(paste0(prefix.string,'venn_deg_k4.pdf'),width=3,height=3)
venn = Venn(deg_k4_list[c('avr','avnr','rvnr')])
...
qtl_master_list = data.frame(
  eqtl_name=c("eqtl_10_1e-9.4","eqtl_12_1e-9.3","eqtl_16_1e-9.4"),
  eqtl_path=c(eqtl_10_path,eqtl_12_path,eqtl_16_path),
  thr=c("1e-9","1e-9","1e-9")
  )
...
genelist_summary = data.table(
  name.exp=c(paste0("var5p.",2:5),
             ...
             paste0("avr.",2:5,".top1"),
             paste0("avr.",2:5,".top2"),
             paste0("avnr.",2:5,".top1"),
             paste0("avnr.",2:5,".top2"),
             paste0("rvnr.",2:5,".top1"),
             paste0("rvnr.",2:5,".top2"),
             "eqtl_10_1e-9.4","eqtl_12_1e-9.3","eqtl_16_1e-9.4"),
  name.display=c(paste0('top 5% varied k',2:5),
                 ...
                 paste0('A vs R top 5% DE k',2:5),
                 paste0('A vs R p<=0.05 DE k',2:5),
                 paste0('A vs NR top 5% DE k',2:5),
                 paste0('A vs NR p<=0.05 DE k',2:5),
...
genelist_summary3 = genelist_summary[
  name.exp %in% c("var5p.4","allgroups.4.top1","allgroups.4.top2",
                  "avr.4.top1","avr.4.top2","avnr.4.top1","avnr.4.top2",
...
genelist_summary4 = genelist_summary[
  name.exp %in% c("eqtl_10_1e-9.4","eqtl_12_1e-9.3","eqtl_16_1e-9.4"),]
...
genelist_summary5 = genelist_summary[
  name.exp %in% c("var5p.4","allgroups.4.top1",
                  "avr.4.top1","avnr.4.top1","rvnr.4.top1"),]
