...
def main():
    ...
    graphics_prefix = os.path.join(out_graphics_dir,'draw_seedsize_plots_2016-11-30-')
    results_prefix = os.path.join(out_dir,'draw_seedsize_plots_2016-11-30-')
    logger = LoggerFactory.get_logger(os.path.join(out_dir,'draw_seedsize_plots_2016-11-30.log'),
    ...
    gid_start,gid_end = geno_reader.getGenoIndex(chrom=4,pos_start=(4,13393142),pos_end=(4,13393144))
    ...
    for ig,g_ID in enumerate(gid_range):
        ...
        plt.subplot(1,2,2)
        plt.plot(esnps_sub+0.05*np.random.randn(esnps_sub.shape[0]),fba5_sub ,'.')
        plt.xlabel("SNP")
